...
type
  ...
  GlobalHotkeyMessageLoop = ref object
    messages: seq[(int, ModifierFlags)]
  ...
  HotkeyListener* = ref object
    ...
    sleep_time: float
    hotkeys: Table[(int, ModifierFlags), int32]
    callbacks: Table[(int, ModifierFlags), proc () {.async.}]
    running: bool
...
proc checkForMessage(self: GlobalHotkeyMessageLoop, keycode: int, modifiers: ModifierFlags): bool =
...
proc messageLoop(self: GlobalHotkeyMessageLoop) {.async.} =
  while self.running:
    ...
    if is_message:
      ...
      self.messages.add((keycode.int, cast[ModifierFlags](modifiers)))
...
proc handleHotkey(self: HotkeyListener, keycode: int, modifiers: ModifierFlags) =
...
proc registerHotkey(self: HotkeyListener, key: int, modifiers: ModifierFlags = {}): bool =
  let
...
proc addHotkey*(self: HotkeyListener, key: int, callback: proc () {.async.}, modifiers: ModifierFlags = {}) = 
...
proc unregisterHotkey(self: HotkeyListener, key: int, modifiers: ModifierFlags = {}): bool =
  let hotkey_id = self.hotkeys[(key, modifiers)]
  result = UnregisterHotKey(0, hotkey_id).bool
  if result:
    hotkey_id_manager.freeId(hotkey_id)

proc removeHotkey*(self: HotkeyListener, key: int, modifiers: ModifierFlags = {}) =
  ## Unregister a hotkey
